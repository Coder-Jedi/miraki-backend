...
export class CartService {
  ...
  async getCart(userId: string): Promise<{ items: CartItem[]; summary: CartSummary }> {
    const items = await this.cartItemModel.find({ userId }).sort({ createdAt: -1 });
    const summary = this.calculateSummary(items);
    
    return { items, summary };
  }

  async addItem(userId: string, addCartItemDto: AddCartItemDto): Promise<{ item: CartItem; cart: { summary: CartSummary } }> {
  ...
  async updateItem(userId: string, itemId: string, updateCartItemDto: UpdateCartItemDto): Promise<{ items: CartItem[]; summary: CartSummary }> {
  ...
  async removeItem(userId: string, itemId: string): Promise<{ items: CartItem[]; summary: CartSummary }> {
